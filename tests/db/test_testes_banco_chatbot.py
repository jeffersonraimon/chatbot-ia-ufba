import pytest
from sqlalchemy import create_engine, inspect, Column, Integer, String, Text, Date, ForeignKey
...
#from models import Base, Instituto, ProgramaAcademico, Disciplina, Ementa, Evento

@pytest.fixture(scope="module")
def engine():
    """Cria e retorna um banco de dados SQLite em memória."""
...
def test_tabelas_criadas(engine, setup_db):
    ...
    assert set(tabelas) == {"instituto", "programaacademico", "disciplina", "ementa", "evento", "localizacao"}
...
def test_inserir_instituto(session, setup_db):
    ...
    session.commit()
    assert session.query(Instituto).filter_by(nome="Instituto de Computação").first() is not None

def test_inserir_programa_academico(session, setup_db):
    """Testa a inserção de um Programa Acadêmico."""
    programa = ProgramaAcademico(nome="Ciência da Computação", descricao="Bacharelado em Ciência da Computação", duracao=4)
    session.add(programa)
    session.commit()
    assert session.query(ProgramaAcademico).filter_by(nome="Ciência da Computação").first() is not None

def test_inserir_disciplina(session, setup_db):
    """Testa a inserção de uma Disciplina vinculada a um Programa Acadêmico."""
    programa = session.query(ProgramaAcademico).filter_by(nome="Ciência da Computação").first()
    disciplina = Disciplina(nome="Inteligência Artificial", programa_academico=programa.id_programa)
    session.add(disciplina)
    session.commit()
    assert session.query(Disciplina).filter_by(nome="Inteligência Artificial").first() is not None

def test_integridade_referencial_ementa(session, setup_db):
    """Testa a integridade referencial ao inserir uma Ementa com Disciplina inexistente."""
    ementa_invalida = Ementa(id_disciplina=999, conteudo="Conteúdo inválido")
    session.add(ementa_invalida)
...
def test_inserir_evento(session, setup_db):
    ...
    evento = Evento(nome="Semana da Computação", descricao="Evento anual sobre tecnologia e inovação.")
    session.add(evento)
    session.commit()
    assert session.query(Evento).filter_by(nome="Semana da Computação").first() is not None
